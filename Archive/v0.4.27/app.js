...
function deleteCurrentEntry() {
  ...
  function ensureGenericConfirmDialog() {
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && genericConfirmBackdrop && !genericConfirmBackdrop.classList.contains("hidden")) {
        closeGenericConfirm();
      }
    });
  ...
  function requestDeleteNotebook(notebookId) {
    if (notebooks.length <= 1) {
      updateStatus("You must keep at least one journal");
      exitNotebookDeleteMode();
      return;
    }
...
function buildTagIndex() {
  ...
  for (const entry of getEntriesForActiveNotebook()) {
...
function runSearchEntriesByNormalizedTag(tagNorm, tagDisplay) {
  ...
  const results = getEntriesForActiveNotebook().filter(e => {
...
function runTagBrowse(fragment) {
  ...
  function runSearch(query) {
    ...
    const results = getEntriesForActiveNotebook().filter(e => entryMatchesQuery(e, qLower));
    results.sort((a, b) => (b.updatedAt || "").localeCompare(a.updatedAt || ""));
